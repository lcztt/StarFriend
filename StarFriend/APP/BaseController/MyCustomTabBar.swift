...
class MyCustomTabBar: UITabBar {
    ...
    public var barBackColor : UIColor = UIColor.hexVal(0x262626, 0.8)
    public var barHeight : CGFloat = 45
    
    public var circleBackColor : UIColor = UIColor.hexVal(0x4b4b4b, 1)
    public var circleRadius : CGFloat = 30
    
    var TabbarMarginBottom : CGFloat = 5
    var tabbarMarginTop : CGFloat = 0
    
    let pitCornerRad : CGFloat = 5
    
    let pitCircleDistanceOffset : CGFloat = 2
    
    private var barRect : CGRect{
        get{
            let tabbarMarginLeft : CGFloat = 12
            let tabbarMarginRight : CGFloat = 12
            
            let h = self.barHeight
            let w = bounds.width - (tabbarMarginLeft + tabbarMarginRight)
            let x = bounds.minX + tabbarMarginLeft
            let y = tabbarMarginTop + circleRadius
    ...
    private func createBackgroundPath() -> CGPath{
        let rect = barRect
        let barTopRadius : CGFloat = 10
        let barBottomRadius : CGFloat = 20
    ...
    override public func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        sizeThatFits.height = self.barHeight + tabbarMarginTop + TabbarMarginBottom + self.circleRadius
        return sizeThatFits
    }
